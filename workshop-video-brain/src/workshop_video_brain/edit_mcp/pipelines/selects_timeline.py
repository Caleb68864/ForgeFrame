...
def build_selects_timeline(
    ...
    for entry in selects:
        ...
        if clip_ref not in seen_producers:
            ...
            # Length covers all use of this source -- approximate as the
            # source asset's duration if known, else this entry's out_point.
            length_frames = (
                int(asset.duration * fps) if (asset and asset.duration) else (out_point + 1)
            )
