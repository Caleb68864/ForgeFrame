...
| 6 | `clips_find_duplicates` | Perceptual/`signature`-hash clips to surface near-duplicate and re-recorded takes | **M** | dedupes `broll_library` -- **BUILT 2026-07-03** (bundles/clip_dupes.py + pipelines/clip_dupes.py; phash dHash + union-find clustering, filter-gated MPEG-7 signature method; JSON report to reports/; verified groups trimmed+re-encoded near-dup ~98.9% sim, excludes different clip) | ✅ BUILT |
| 7 | `clips_preview_gif` | Generate a small looping GIF/mp4 preview per clip for fast visual triage | **S** | review UI / index -- **BUILT 2026-07-03** (bundles/clip_preview.py + pipelines/clip_preview.py; two-pass palettegen/paletteuse GIF or tiny mp4 to reports/previews/; verified 3s@8fps@320w -> 320x240, 24 frames) | ✅ BUILT |
