...
# Scenario AP-02: MCP Resources

## Description
Tests the MCP resource handler functions in `edit_mcp/server/resources.py`.

Each resource is a plain Python function decorated with `@mcp.resource(...)`.
Because the decorators run at import time, tests call the underlying functions
directly — no MCP transport is involved.

Resources under test:

| Function | URI | What it returns |
|----------|-----|-----------------|
| `workspace_current_summary` | `workspace://current/summary` | Manifest formatted as markdown |
| `workspace_media_catalog` | `workspace://{id}/media-catalog` | Media asset list |
| `workspace_transcript_index` | `workspace://{id}/transcript-index` | Transcript summaries |
| `workspace_markers` | `workspace://{id}/markers` | Marker report table |
| `workspace_timeline_summary` | `workspace://{id}/timeline-summary` | kdenlive file listing |
| `workspace_validation` | `workspace://{id}/validation` | Validation report |
| `workspace_render_logs` | `workspace://{id}/render-logs` | Render job status |
| `system_capabilities` | `system://capabilities` | Tool/adapter status doc |

Edge cases: missing directories return a human-readable error string (not a
raised exception).  `_workspace_summary_text` returns a "not found" message
when `workspace.yaml` is absent.

## Preconditions
- Python 3.12+, `uv run pytest` available.
- `tmp_path` used to build minimal on-disk workspace structures.
- `workspace.yaml` seeded via `WorkspaceManifest` / `write_manifest` for
  `workspace_current_summary` tests.
- No real FFmpeg, melt, or whisper required.
...
class TestWorkspaceCurrentSummary:
    ...
    def test_fallback_when_no_env_var(self, tmp_path: Path):
        ...
        with patch.dict(os.environ, env, clear=True):
            result = workspace_current_summary()
        # Should return a string (error or content) without raising
        assert isinstance(result, str)
...
class TestWorkspaceMediaCatalog:
    ...
    def test_returns_string_type(self, tmp_path: Path):
        ws = _seed_workspace(tmp_path)
        # media/raw exists but is empty — mock scan_directory to return []
        with patch(
            "workshop_video_brain.edit_mcp.server.resources."
...
## Steps
1. Read source module at `workshop-video-brain/src/workshop_video_brain/edit_mcp/server/resources.py`
2. Create `tests/unit/test_mcp_resources.py`
3. Implement test cases above
4. Run: `uv run pytest tests/unit/test_mcp_resources.py -v`

## Expected Results
- All resource functions return `str` without raising.
- Missing directories produce descriptive error strings rather than exceptions.
- `_workspace_summary_text` returns markdown content when `workspace.yaml` exists.
- `workspace_markers` correctly parses and tabulates a valid `*_markers.json` file.
- `system_capabilities` reports FFmpeg, melt, and faster-whisper status, and lists tools.
- `workspace_current_summary` reads `WVB_WORKSPACE_ROOT` env var when set.
