...
# Scenario AD-02: Render Jobs

## Description
Tests `create_render_job` and `update_job_status` in
`edit_mcp/adapters/render/jobs.py`.

`create_render_job` builds a fresh `RenderJob` (status=`queued`), deriving the
workspace ID from `workspace.yaml` when present, generating a UUID when absent.
It also creates the `renders/` directory and sets `output_path` and `log_path`.

`update_job_status` returns a **new** immutable `RenderJob` with updated
`status`, `started_at`, and `completed_at` timestamps — it must not mutate the
original.

## Preconditions
- Python 3.12+, `uv run pytest` available.
- `tmp_path` used for isolated filesystem; no real render binaries needed.
- `WorkspaceManifest` / `write_manifest` used to seed a workspace for
  manifest-read tests.

## Test Cases

```python
# tests/unit/test_render_jobs.py
...
## Steps
1. Read source module at `workshop-video-brain/src/workshop_video_brain/edit_mcp/adapters/render/jobs.py`
2. Create `tests/unit/test_render_jobs.py`
3. Implement test cases above
4. Run: `uv run pytest tests/unit/test_render_jobs.py -v`

## Expected Results
- `create_render_job` returns a `RenderJob` with `status=queued` and valid paths.
- `renders/` directory is created under the workspace root.
- Workspace ID is read from `workspace.yaml` when present; otherwise a fresh UUID is generated without raising.
- `update_job_status` returns a new `RenderJob` — original is unchanged.
- `started_at` is populated only on the first transition to `running`.
- `completed_at` is populated on `succeeded`, `failed`, or `cancelled`.
- Passing a string status coerces to `JobStatus`; invalid string raises `ValueError`.
