...
# Scenario WS-02: Workspace Folders

## Description
Tests `create_workspace_structure` and `validate_workspace_structure` in
`workspace/folders.py`.

`WORKSPACE_FOLDERS` defines 14 canonical sub-directories (e.g. `media/raw`,
`projects/snapshots`, `renders/final`).  The two functions create and validate
these directories respectively.

Key edge cases:
- Creating on an already-populated directory (idempotent — no error).
- `validate_workspace_structure` returns an empty list when all dirs exist, and
  returns the correct missing entries when some are absent.
- Extra directories not in `WORKSPACE_FOLDERS` are silently ignored by
  `validate_workspace_structure`.
- Root path does not need to pre-exist before `create_workspace_structure` is
  called (it should create everything with `mkdir(parents=True)`).

## Preconditions
- Python 3.12+, `uv run pytest` available.
- `tmp_path` provides isolated directories; no external tools required.

## Test Cases

```python
# tests/unit/test_workspace_folders.py
...
## Steps
1. Read source module at `workshop-video-brain/src/workshop_video_brain/workspace/folders.py`
2. Create `tests/unit/test_workspace_folders.py`
3. Implement test cases above
4. Run: `uv run pytest tests/unit/test_workspace_folders.py -v`

## Expected Results
- All 14 standard folders are created on a fresh root.
- Calling `create_workspace_structure` twice does not raise and leaves all folders intact.
- `validate_workspace_structure` returns `[]` for a complete workspace.
- Removed folders appear in the return value of `validate_workspace_structure`.
- Extra directories are silently ignored.
- `WORKSPACE_FOLDERS` contains no duplicates and includes the safety-critical paths (`media/raw`, `projects/source`).
