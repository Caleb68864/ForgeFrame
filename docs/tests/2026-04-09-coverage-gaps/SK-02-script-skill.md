...
# Scenario SK-02: Script Skill

## Description
Tests `generate_script` in `production_brain/skills/script.py`.

`generate_script(outline_data, tone, target_length)` takes a dict produced by
`outline.generate_outline` (or any compatible dict) and returns
`(markdown_string, structured_dict)`.

Dict keys in the output: `title`, `tone`, `target_length`, `hook`, `overview`,
`materials_section` (with `materials` and `tools` sub-lists), `steps` (list),
`common_mistakes` (list), `conclusion`.

Key test areas:
- Correct dict keys and types returned.
- `steps` list has one entry per `teaching_beat` in the input.
- Each step dict has `number`, `title`, `direction`, `script_text`, `key_points`, `common_mistake`.
- Empty `teaching_beats` → `steps` is an empty list (no crash).
- `tone` kwarg is stored; `target_length` stored and used in conclusion.
- Empty / minimal outline dict (all missing keys) does not raise.
- Markdown contains expected section headings (HOOK, PROJECT OVERVIEW, CONCLUSION, STEP N).
- `_pacing_for_tone` mapping covers `practical`, `educational`, `casual`, and unknown tones.

## Preconditions
- Python 3.12+, `uv run pytest` available.
- `generate_outline` is used as a helper to produce valid input; alternatively
  a minimal dict is constructed inline.
...
## Steps
1. Read source module at `workshop-video-brain/src/workshop_video_brain/production_brain/skills/script.py`
2. Create `tests/unit/test_script_skill.py`
3. Implement test cases above
4. Run: `uv run pytest tests/unit/test_script_skill.py -v`

## Expected Results
- `generate_script` returns a `(str, dict)` tuple with all required keys.
- `steps` count equals `teaching_beats` count in the input.
- Each step dict has `number`, `title`, `direction`, `script_text`, `key_points`, `common_mistake`.
- Empty outline dict does not raise; `steps` is empty when `teaching_beats` is empty.
- `tone` and `target_length` are stored and reflected in markdown metadata.
- All major markdown sections (`HOOK`, `PROJECT OVERVIEW`, `CONCLUSION`, `STEP N`) are present.
