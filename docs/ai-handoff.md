...
10. **melt exits 0 on footage it cannot open**, and the guard against that is a precondition with a deliberately narrow guarantee. melt does not fail when a producer's file is gone: it logs a warning, renders that clip as blank frames, and returns 0. Nothing downstream can tell that from a good render. `adapters/render/media_check` is the one implementation that closes it — it parses the XML about to be rendered and refuses, naming each absent file — and it is reached by every path that hands melt a *project*: `execute_render` (so every render and preview), `pipelines/render_final`, `bundles/subtitle_track`'s burn-in, and `pipelines/review_loop`'s project thumbnails. `adapters/kdenlive/validator.validate_project` answers the same question through the same code, so the advisory report and the hard refusal cannot disagree (`tests/unit/test_media_check_reconciled.py` is the case table that pins it).
    ...
    - Only producers whose `mlt_service` is a known file-backed service (`avformat`, `avformat-novalidate`, `qimage`, `pixbuf`, `timewarp`, `xml`) are checked. It is an allowlist, never a deny-list, because a timeline legitimately contains producers with no file at all (every Kdenlive project carries `producer_black`). An *unrecognised* service is skipped, so the check can never refuse a render it does not understand. An **absent** `mlt_service` is not skipped: `media_check.effective_service` defaults a producer that names a file and omits the service, because melt opens it either way — the serializer writes the service in, and MLT's `loader` picks a demuxer from the resource.
    - Remote resources (`http://`, `smb://`, …), `MLT_DATA`-relative `%` names and image sequences (`%04d`, `.all.`, `glob:`) are skipped: `Path.exists()` cannot settle any of them.
    - **File references that are not producer resources** go through a second allowlist, `FILE_BACKED_PROPERTIES`, keyed by `(element tag, mlt_service)`: the `luma`/`composite` transitions' mattes, `shape` and its `mask_start` sandwich form, the `avfilter.subtitles` sidecar, the `avfilter.lut3d` LUT. Same discipline and the same legitimate absent states — an empty `resource` on a `luma` transition is a plain dissolve, and a bare name with no separator is one of MLT's built-in lumas. Four path-shaped properties are deliberately **not** checked: an `affine` filter's `producer.resource` (it holds a colour), `kdenlive:originalurl` and `kdenlive:proxy` (proxy bookkeeping melt never opens), and `kdenlive:docproperties.renderurl` (an output path). A file named inside a `kdenlivetitle`'s `xmldata` (a `QGraphicsPixmapItem`'s `<content url=…>`) is not covered; nothing here writes one.
...
11. **A save preserves the imported `<mlt root>`; it never rewrites resource strings.** `KdenliveProject.root` is the directory a project's relative `resource` values resolve against — the `<mlt root>` attribute, or the document's own directory when that attribute is absent or empty. `media_check.effective_root` is the single statement of that rule and both front doors call it: `missing_media` (the XML door) and `parse_project` (the model door, which stores the *effective* root, not the raw attribute). `serialize_project` writes it back verbatim, so importing a project with relative resources and saving it elsewhere keeps those resources naming the same files. Rewriting the paths instead was rejected: MLT `resource` values are not all filesystem paths (`color`/`black`, built-in luma names, `%`-prefixed `MLT_DATA` names, `%04d`/`glob:` sequences, bare clip ids), and a rewriter that misidentifies one corrupts the project silently. `root` is empty only for a project built in memory, where the output file's own directory is still the right answer. **To relocate** a project deliberately, set `project.root` before saving (or clear it to re-root on the output directory) — that is the escape hatch, and both directions are covered by accept controls in `tests/unit/test_kdenlive_root_roundtrip.py`. There is no relink yet: an absolute root imported from another machine is preserved as-is.
