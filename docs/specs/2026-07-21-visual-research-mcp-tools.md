...
- **Acceptance criteria:**
  ...
  - `[BEHAVIORAL]` `research_extract_frame` at `t=1.0` on the fixture writes a nonempty PNG and returns `status="ok"` with `data.actual_timestamp_seconds` present in the envelope; `abs(actual_timestamp_seconds - 1.0) <= 0.05` (frame-grain tolerance). A timestamp past EOF (e.g., t=10000.0 on a 10-second fixture) is clamped to the last valid frame, and `actual_timestamp_seconds` reflects the clamped position.
...
- **Acceptance criteria:**
  ...
  - `[INTEGRATION]` Full handshake E2E: `research_generate_candidates` → parse `candidates.json` → `research_select_candidate(cand-001)` → package exists on disk with `index.md` + `manifest.json` + `screenshots/*.png`. Assert: `manifest['captures'][0]['timestamp_seconds']` == the `timestamp_seconds` value of the candidate with id `cand-001` from `candidates.json`, within floating-point tolerance (epsilon 1e-6).
  - `[STRUCTURAL]` The integration test validates that the `candidates.json` written by SS-03 conforms to schema v1 (all required keys present, candidate IDs follow `cand-NNN` format, source fingerprint fields present and match the input video at test time, image_path values point to existing PNG files). This is the schema contract that SS-04 and SS-05 depend on for rehydration.
